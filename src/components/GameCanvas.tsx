...
const NEON = ["#4df3ff", "#ff4dd8", "#7dff4d", "#ffd24d", "#a24dff"]
const ROLES = ["SCOUT", "SENTRY", "COURIER", "DRONE", "RELAY", "WARDEN"]
const FLAVORS = [
...
export default function GameCanvas() {
  ...
  const [activeBotsCount, setActiveBotsCount] = useState(BOT_COUNT)
  ...
  useEffect(() => {
    ...
    class Bot {
      ...
      constructor(id: number) {
        ...
        this.color = NEON[id % NEON.length]
        this.pauseTimer = rand(20, 80)
        this.size = 13
        this.label = "NPC_" + String(id + 1).padStart(2, "0")
        this.bob = rand(0, Math.PI * 2)
        this.role = ROLES[id % ROLES.length]
      ...
      currentHeight() {
        const h1 = heights[this.fromRow]
          ? heights[this.fromRow][this.fromCol]
          : 0
    ...
    const bots = Array.from({ length: BOT_COUNT }, (_, i) => new Bot(i))
    ...
    const drawScene = () => {
      ...
      sorted.forEach((b) => b.draw(b === selectedBot))
    ...
    const handleTap = (clientX: number, clientY: number) => {
      ...
      let closest: Bot | null = null
      let closestDist = 26

      for (const bot of bots) {
        const dx = bot.worldX - world.x
        const dy = bot.worldY - world.y
